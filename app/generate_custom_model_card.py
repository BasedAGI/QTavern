...
{f"""
### What is a GGUF?
GGUF is a file format used for running LLMs (large language models) on different types of computers. It works on both regular processors (CPU) and graphics cards (GPU), making it easier to run models on a wider range of hardware. Many LLMs require powerful and expensive GPUs, but GGUF improves compatibility and efficiency by optimizing how models are loaded and executed. If a GPU doesn’t have enough memory, `GGUF can offload parts of the model to the CPU`, allowing it to run even when GPU resources are limited. GGUF is designed to work well with quantized models, which use less memory and run faster, making them ideal for lower-end hardware. However, GGUF can also store full-precision models when needed. Thanks to these optimizations, GGUF allows LLMs to run efficiently on everything from high-end GPUs to laptops and even CPU-only systems.
""" if "GGUF" in formatted_qtype else ""}
{f"""
### What is an i1-GGUF?
i1-GGUF is a type of GGUF model that uses `imatrix quantization`, a smarter way of reducing model size while keeping key details intact. Instead of shrinking everything equally, it figures out which parts of the model are most important and keeps them more accurate. Like GGUF, it allows large language models (LLMs) to run on different computers, including CPUs and lower-end GPUs. But because it prioritizes important weights, i1-GGUF models give better responses than regular GGUF models.""" if formatted_qtype == "i1-GGUF" else ""}
"""
